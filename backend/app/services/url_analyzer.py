...
from score_engine import calculate_score, generate_recommendation, generate_summary
import sys
import os
...
def analyze_url(url: str) -> dict:
    ...
    result["recommendations"] = generate_recommendation(result)
    ...
    try:
        ...
        result["ml_is_phishing"] = ml_result["ml_is_phishing"]
        if ml_result["ml_is_phishing"]:
            result["risk_score"] = min(100, result["risk_score"] + 20)
            result["details"].append(f"🤖 IA détecte du phishing ({ml_result['ml_confidence']}% de confiance)")
        else:
            result["details"].append(f"🤖 IA considère ce lien légitime ({ml_result['ml_confidence']}% de confiance)")
    except Exception as e:
        result["details"].append("⚠️ Analyse ML indisponible")
    return result
